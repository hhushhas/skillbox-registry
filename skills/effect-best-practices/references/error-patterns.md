...
// Callers can handle each case by tag:
Effect.catchTags({
    UserNotFoundError: (err) => handleMissingUser(err.userId),
    ChannelNotFoundError: (err) => handleMissingChannel(err.channelId),
    SessionExpiredError: () => refreshSession,
})
```
...
Prefer `Schema.TaggedError` for defining errors. This provides:

1. **Serialization** - Errors can be sent over API/network boundaries
...
Most domain errors should have:
...
Avoid `catchAll` or `mapError` when you can use `catchTag`/`catchTags`. These preserve type information and enable precise error handling.
...
```typescript
// Preferred - Domain errors with HTTP status annotations
export class UserNotFoundError extends Schema.TaggedError<UserNotFoundError>()(
    "UserNotFoundError",
    { userId: UserId, message: Schema.String },
...
```typescript
// Avoid - Generic HTTP error classes
export class UnauthorizedError extends Schema.TaggedError<UnauthorizedError>()(
    "UnauthorizedError",
    { message: Schema.String },
    HttpApiSchema.annotations({ status: 401 }),
...
// Then mapping everything to it loses useful information.
Effect.catchTags({
...
Generic errors are best reserved for unrecoverable internal errors where:
...
// Use sparingly for unexpected errors.
