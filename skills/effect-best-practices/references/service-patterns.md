...
## Effect.Service For Owned Business Services

Prefer `Effect.Service` for owned business logic services. This gives the codebase a consistent shape:
...
4. **Consistent structure** - Services follow the same broad pattern

Use `Context.Service` when the service contract and implementation should be intentionally separate. This is useful for adapters, ports, and replaceable integrations where callers should depend on a stable interface rather than the live implementation.
...
Declare business dependencies using the `dependencies` array. This helps ensure:
...
Wrap service methods with `Effect.fn`. This provides automatic tracing with meaningful span names.
...
## When Context.Service is Acceptable

Use `Context.Service` for explicit service contracts, especially when the implementation is replaceable or lives in a separate layer file:
...
export const NodePtyAdapterLive = Layer.effect(
    ...
    Effect.gen(function* () {
        return {
            spawn: Effect.fn("PtyAdapter.spawn")(function* (input) {
                // live adapter code
            }),
...
This is different from using `Context.Tag` for owned business services. `Context.Service` still gives the service a named contract and typed layer target.

## When Context.Tag is Acceptable

`Context.Tag` is still appropriate for infrastructure that is injected at runtime:
...
Services should generally return `Effect` types rather than `Promise`:
